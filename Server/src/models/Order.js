...
const orderSchema = new mongoose.Schema(
  {
    ...
    // Meeting info for deposit orders (admin can schedule time/place)
    meeting: {
      ...
      isSuggestion: { type: Boolean, default: false }, // true = buyer suggestion, false = admin confirmed
    },
...
orderSchema.pre('save', async function (next) {
  ...
  if (this.shipping?.method && this.shipping.method !== 'in-person') {
    ...
    const missingFields = requiredFields.filter(
      (field) => !this.shippingAddress?.[field]
    );

    if (missingFields.length > 0) {
      const err = new Error(
        `Shipping address incomplete: missing ${missingFields.join(', ')}`
      );
